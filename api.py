...
def get_news(company_req: CompanyRequest):
    ...
    # Extract news headlines from articles
    ...
    # Translate final sentiment analysis to Hindi
    final_sentiment_hindi = translate_to_hindi(final_sentiment_english)

    # Generate Hindi speech file from the Hindi text
    logging.debug("Generating TTS for Hindi sentiment analysis.")
    audio_file, hindi_text = text_to_speech(final_sentiment_english, lang="hi")
    audio_indicator = "[Play Hindi Speech]" if audio_file and os.path.exists(audio_file) else "TTS Generation Failed"
...
def generate_tts(request: TextRequest):
    """
    Receives English text, translates it to Hindi, and generates a Hindi speech MP3 file.
    """
